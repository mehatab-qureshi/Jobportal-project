...
const  AdminJobsTable = () => {
  const {allAdminJobs =[],searchJobByText} = useSelector(store => store.job);  //5

  //ab filter ke liye state..
  const [filterJobs, setFilterJobs] = useState(allAdminJobs); //6
  ...
  useEffect(() => {
    ...
    allAdminJobs.filter((job) => { 
        if (!searchJobByText) { 
          return true; //vhi jobs display hogi jo already hai... 
        }
        return job?.title
          ?.toLowerCase()
